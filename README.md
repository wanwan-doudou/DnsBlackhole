...
- 支持查询日志，可按已处理、已过滤、失败筛选，并按域名或客户端搜索；拦截详情会显示命中规则、来源清单、规则类型、`important` 覆盖和 allowlist
- 支持客户端名称映射（`IP 名称`），查询日志用设备名代替 IP 展示
- 安全防护页展示访问拒绝、限速和 UDP 静默丢弃统计，并在内存中保留最近 200 条聚合安全事件
...
- 默认同时监听 `0.0.0.0:53` 和 `[::]:53`，可在 DNS 设置中关闭 IPv6 监听；默认只允许本机、私有 IPv4 网段和常见内网 IPv6 网段客户端访问
...
- DoH 和域名形式的 UDP 上游通过 Bootstrap DNS 并行解析 A/AAAA，并使用多个解析地址回退。
- 不做 DNSSEC 验证。
...
默认同时监听 `0.0.0.0:53` 和 `[::]:53`，但默认访问控制只允许本机和私有网络客户端访问。不需要修改系统 DNS 时，可以直接从本机回环地址测试：
